...
@pytest.mark.parametrize(
    "config,timm_name",
    [
        (dict(variant="Ti/16", img_size=224, weights="augreg"), "vit_tiny_patch16_224.augreg_in21k"),
        (dict(variant="B/16", img_size=224, weights="siglip"), "vit_base_patch16_siglip_224"),
    ],
)
def test_from_pretrained(config, timm_name):
    m = ViT.from_google(**config).eval()
